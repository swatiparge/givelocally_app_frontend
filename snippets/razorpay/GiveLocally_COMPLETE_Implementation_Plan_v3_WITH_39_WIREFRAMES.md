...
# GiveLocally_COMPLETE_Implementation_Plan_v3_WITH_39_WIREFRAMES.md
...
│   └── Design Principles
│
├── 2. 🆕 COMPLETE UI/UX WIREFRAMES (39 SCREENS)
│   ├── Section A: Auth & Onboarding (4)
│   ├── Section B: Discovery (3)
│   ├── Section C: Donation Posting (8)
│   ├── Section D: Detail & Interaction (4)
│   ├── Section E: Payment & Pickup (8) ⭐ 3 VERIFICATION
│   ├── Section F: User Profile (7)
│   ├── Section G: Additional Features (5)
│   ├── Implementation Checklist
│   └── Design Tokens
│
├── 3. System Architecture
├── 4. Database Schema (12 collections)
├── 5. Firestore Security Rules
├── 6. 14-Week Implementation Timeline
├── 7. Payment & Pickup Flows
├── 8. Admin Dashboard & Blood Verification
├── 9. Cost Optimization
├── 10. Testing & QA
├── 11. Launch Preparation
└── 12. Final Developer Handover
...
### 5.6 APIS and Missing 


Based on your implementation plan, here are **all the remaining Cloud Functions** you need to create: 

## Authentication Functions ✅ (Complete)
- `sendOTP` - ✅ Done
- `verifyOTP` - ✅ Done

## Donation Functions (Partial)
- `createDonation` - ✅ Done
- `updateDonationStatus` - ✅ Done
- `getNearbyDonations` - ✅ Done
- ❌ **Missing:** `expireDonations` (scheduled function)

## Claim & Request Functions ❌ (All Missing)
```typescript
// 1. Create a request to claim a donation
export const createRequest = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{donationId: string, message: string}>) => {
...
// 2. Donor accepts a specific receiver
export const acceptRequest = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{donationId: string, requestId: string}>) => {
...
// 3. Get requests for donor's donation
export const getDonationRequests = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{donationId: string}>) => {
    // Verify caller is donor
...
## Payment Functions ❌ (All Missing)
```typescript
// 4. Create Razorpay order (auth-only, no capture)
export const createPaymentOrder = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{donationId: string, idempotencyKey: string}>) => {
    // Check idempotency key
    // Validate donation is active and claimed by this user
    // Create Razorpay order with payment_capture: 0
    // Save to idempotencyKeys collection
...
export const razorpayWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    //   - Generate 4-digit pickupCode
    //   - Create transaction document (PRIVATE)
    //   - Update donation: status=reserved, addressVisible=true
    //   - Send FCM with pickup code and address
    // Return 200 OK
  }
);
```

## Pickup Verification Functions ❌ (All Missing)
```typescript
// 6. Generate pickup OTP (optional, if using OTP instead of auto-code)
export const generatePickupOTP = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{donationId: string}>) => {
    // Verify caller is donor
    // Generate 4-6 digit OTP
    // Save to transaction with expiry
    // Send SMS to donor (optional)
    // Return OTP
  }
);

// 7. Verify pickup code (donor enters, completes transaction)
export const verifyPickupCode = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{donationId: string, code: string}>) => {
    // Verify caller is donor
    // Find transaction by donationId
    // Validate code matches and not expired/used
    // Mark pickupCodeUsed = true
    // Cancel Razorpay authorization (refund ₹50)
...
## Auto-Forfeit Scheduled Function ❌ (Missing)
```typescript
// 8. Auto-capture expired authorizations (runs every 10 minutes)
export const checkExpiredAuthorizations = onSchedule(
  ...
  async (event) => {
    ...
    // For each: try to capture ₹50
    //   - Success: paymentStatus = "captured"
    //   - Failure (expired auth): paymentStatus = "expired"
    // Apply trust penalties to receiver
    // Return donation to active status
    // Send notifications
  }
);
```

## Chat Functions ❌ (Missing)
```typescript
// 9. Send chat message
export const sendMessage = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{donationId: string, requestId: string, message: string}>) => {
    // Validate user is donor or receiver
    // Create message in donations/{donationId}/requests/{requestId}/messages
    // Send FCM to other party
  }
);
```

## Review & Rating Functions ❌ (Missing)
```typescript
// 10. Submit review after pickup
export const submitReview = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{transactionId: string, rating: number, comment: string}>) => {
    // Validate transaction is completed
    // Verify user is donor or receiver
...
## Dispute Functions ❌ (Missing)
```typescript
// 11. Create dispute
export const createDispute = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{transactionId: string, reason: string, evidence: string[]}>) => {
    // Validate transaction exists
    // Create dispute document with status: "pending"
    // Send notification to admin
  }
);

// 12. Admin resolve dispute
export const resolveDispute = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{disputeId: string, decision: string, refundReceiver: boolean}>) => {
    // Check admin permissions
    // Update dispute status
    // If refundReceiver: process Razorpay refund
...
```typescript
// 13. Approve blood request
export const approveBloodRequest = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{donationId: string, approved: boolean, reason?: string}>) => {
    // Check admin permissions
    // Update donation: verificationStatus = "approved" or "rejected"
...
// 14. Get pending blood requests (admin only)
export const getPendingBloodRequests = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{}>) => {
    // Check admin permissions
    // Query donations where category="blood" and verificationStatus="pending"
    // Return list with proof documents
  }
);
...
export const banUser = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{userId: string, banned: boolean, reason: string}>) => {
    // Check admin permissions
    // Update user: isBanned, banReason
    // Expire all active donations
    // Cancel all pending claims
  }
);
```

## User Profile Functions ❌ (Missing)
```typescript
// 16. Update profile
export const updateProfile = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{name: string, bloodGroup: string, profilePicture?: string}>) => {
    // Validate input
    // Update users document
  }
);

// 17. Get public user profile
export const getUserProfile = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{userId: string}>) => {
    // Return public profile data
    // Include karma, ratings, completed donations
  }
);
...
export const getLeaderboard = onCall(
  { region: "asia-southeast1" },
  async (request: CallableRequest<{period: "month" | "allTime"}>) => {
    // Query leaderboard collection
    // Return top users by karma
  }
);
```
...
| Category | Functions Needed | Priority |
|----------|-----------------|----------|
| Claim/Request | 3 | **HIGH** - Core flow |
| Payment | 2 | **HIGH** - Core flow |
| Pickup | 2 | **HIGH** - Core flow |
| Auto-forfeit | 1 | **HIGH** - Revenue critical |
| Chat | 1 | **MEDIUM** |
| Reviews | 1 | **MEDIUM** |
| Disputes | 2 | **MEDIUM** |
| Admin | 3 | **MEDIUM** |
...
You've completed **3 out of 21** Cloud Functions (14%). The highest priority is implementing the **claim → payment → pickup verification** flow to complete the core donation cycle. 
